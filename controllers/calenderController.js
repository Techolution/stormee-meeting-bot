...
const calenderNotificationController = async (req, res) => {
  {
    ...
    if (resourceState === "sync") {
      console.log(
        "This is a sync message - watch channel successfully established"
      );
      ...
      try {
        ...
        if (events && events.length > 0) {
          ...
          console.log(
            `Found the most recently updated event: ${event.summary}`
          );
          ...
          console.log(
            "Current queue count:",
            await meetingQueue.getJobCounts()
          );
...
export { calenderNotificationController}